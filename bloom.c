...
{
  int isIn = 0;//0 if elm is in, 1 o.w.

  //Calculate the hashes, then see if the digit is marked
  for(int i =0; i< BLOOM_HASH_NUM; i++){
    int hash = hash_i(i,elm);// the i-th hash of elm
    hash %= f->bsz;
    char* target = f->buf;// the char position to check
    target+=(int) (hash/8);
    if(!(isMark(target,(int)(hash%8))))
      isIn = 1;
  }
...
bloom_bit_at_pos(bloom_filter *bf, int pos)
{
	...
	return bf->buf[which_byte] & (1<< (7-which_bit));
}
